...
local function sendFeedback(txt)
    feedback.Text = tostring(txt)
end

local function findPlayerByName(partial)
    partial = partial:lower()
    for _,p in pairs(Players:GetPlayers()) do
        if p.Name:lower():find(partial) or (p.DisplayName and p.DisplayName:lower():find(partial)) then
            return p
        end
    end
    return nil
    ...
    flingConnection = RunService.Heartbeat:Connect(function()
        ...
        if char and targetChar and char:FindFirstChild("HumanoidRootPart") and targetChar:FindFirstChild("HumanoidRootPart") then
            ...
            if humanoid and humanoid.Sit then
                hrp.CFrame = CFrame.new(Vector3.new(5516134400, 196881252352, 10508566528))
                stopFling()
                return
            end
